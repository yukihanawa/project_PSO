...
class Field:
    ...
    def maintain_diversity(self, i):
        # If the standard deviation is less than the threshold, reinitialize some particles
        if i == MAX_ITERATION//4:
            # Select a half of the particles randomly
            selected_particles = np.random.choice(self.particles, size=self.N//2, replace=False)

            for particle in selected_particles:
                particle.position = np.random.uniform(POS_MIN, POS_MAX, self.D)
                particle.velocity = np.zeros(self.D)
                particle.pbest = particle.position.copy()
                particle.pbest_fitness = math.inf
                particle.gbest = self.gbest
...
for function in function_set:
    for  w, c1, c2 in parameter_set:

        output_directory = f'./{function}_{w}_{c1}_{c2}_replace' #出力先のディレクトリを作成
        ...
        for seed in range(11):
            ...
            #ファイル名を作成
            filename = f'{function}_seed{seed}_replace.csv'
